...
export async function POST(req:Request){
  try{
    ...
    const question=`Create a ${period} founder competitive-intelligence briefing for ${workspace.name}. Start from RADAR's stored evidence, then use current public-web research to check whether any monitored competitor or important adjacent company has made a material new move. Structure it as: Executive summary; Important verified changes; New or emerging competitors; Market themes; Recommended founder actions; Confidence and evidence gaps. Exclude generic news and cosmetic activity. Every current claim should be grounded in a public source or explicitly labeled as inference.`;
    const ai=await generateRadarAnswer({question,workspace,competitors,signals,recommendations,evidence,feature:`briefing_${period}`,liveWeb:true});
    const content=ai||[
    ...
    const now=new Date();
    const created=await sbInsert("radar_briefings",{workspace_id:workspace.id,period,title:`${period[0].toUpperCase()+period.slice(1)} founder briefing · ${now.toLocaleDateString("en-GB")}`,content,evidence_count:evidence.length,generated_by:ai?"openrouter-live-grounded-ai":"evidence-fallback",period_end:now.toISOString()});

...
const genericFragments=new Set(["skills","age groups","learning conditions","conditions","features","capabilities","technology","technologies","customers","users","buyers","market"]);

function rawParts(value:unknown):string[]{
...
function conciseTerms(value:unknown,limit=30):string[]{
  const out:string[]=[];
  for(const part of rawParts(value)){
    const clean=part.replace(/\s+/g," ").trim();
    const words=clean.split(/\s+/).filter(Boolean);
    const normalized=clean.toLowerCase().replace(/[^a-z0-9 ]/g," ").replace(/\s+/g," ").trim();
    if(clean.length<3||clean.length>96||words.length>12||genericFragments.has(normalized))continue;
    if(!out.some(x=>x.toLowerCase()===clean.toLowerCase()))out.push(clean);
    if(out.length>=limit)break;
  }
  return out;
}
function tokens(value:unknown){return [...new Set(rawParts(value).flatMap(x=>x.toLowerCase().split(/[^a-z0-9]+/)).filter(x=>x.length>=4))]}
...
function validCompanyName(value:string){const name=cleanCompanyName(value);return name.length>=3&&/[a-z]{2,}/i.test(name)&&!/^https?:/i.test(name)&&!genericFragments.has(name.toLowerCase())}
function cleanQuery(value:string){const q=String(value||"").replace(/\s+/g," ").trim();return q.length>=8&&q.length<=180?q:""}
function scoreGroup(text:string,values:unknown,weight:number){
  const phrases=conciseTerms(values,40).map(x=>x.toLowerCase());const toks=tokens(values);
...
function baseQueries(workspace:any){
  ...
  const product=conciseTerms(workspace.product_keywords,8).filter(x=>!own||!x.toLowerCase().includes(own)).slice(0,5);
  const features=conciseTerms(workspace.major_features,8).slice(0,5);
  const caps=conciseTerms(workspace.capability_keywords,10).slice(0,6);
  const industry=conciseTerms(`${workspace.industry||""},${workspace.sub_category||""}`,8).slice(0,4);
  const geography=conciseTerms(workspace.geography,4).slice(0,1);
  const q=new Set<string>();
  if(product[0]){
    q.add(`${product[0]} competitors`);
    q.add(`${product[0]} alternatives software`);
    q.add(`${product[0]} startup platform`);
  }
  if(product[0]&&product[1])q.add(`${product[0]} ${product[1]} company`);
  if(industry[0])q.add(`${industry.slice(0,2).join(" ")} startups`);
  for(const cap of [...features,...caps].slice(0,5))q.add(`"${cap}" software platform`);
  if(product[0]&&geography[0])q.add(`${product[0]} startup ${geography[0]}`);
  return [...q].map(cleanQuery).filter(Boolean).slice(0,10);
}

const sourceDomains=["wikipedia.org","ycombinator.com","crunchbase.com","tracxn.com","wellfound.com","linkedin.com","youtube.com","reddit.com","news.mit.edu","mit.edu","caltech.edu","techcrunch.com","forbes.com","reuters.com","bloomberg.com","medium.com","cbinsights.com","g2.com","capterra.com","producthunt.com","ncbi.nlm.nih.gov","pmc.ncbi.nlm.nih.gov","dl.acm.org","researchgate.net","arxiv.org","springer.com","nature.com","sciencedirect.com","frontiersin.org","aws.amazon.com","oracle.com","edtechimpact.com","edcircuit.com","nbcnews.com","educationweek.org"];
function sourceLike(domain:string){return sourceDomains.some(x=>domain===x||domain.endsWith(`.${x}`))}
function articleLike(result:any){const d=domainOf(result.url);const text=`${result.title||""} ${result.description||""}`.toLowerCase();return sourceLike(d)||/(top\s+\d+|best\s+.*companies|startups funded|news|review|list of|companies to watch|researchers|study|report|journal|paper|guide|introduction to|trends|doi|research)/i.test(text)}
...
async function resolveOfficialWebsite(name:string,relatedProduct:string,supplied:string,ownDomain:string){
  ...
  try{
    const productHint=conciseTerms(relatedProduct,2)[0]||"product";
    const results=await searchAll(cleanQuery(`"${name}" ${productHint} official`)||`"${name}" official`,5);
...
export async function GET(req:Request){
  try{
    ...
    const [candidates,expansionRows,recentRuns,monitors]=await Promise.all([
      ...
      sbSelect(`radar_scan_runs?workspace_id=eq.${workspace.id}&run_type=eq.market_discovery&select=id,status,pages_scanned,findings,error,started_at,finished_at&order=started_at.desc&limit=5`).catch(()=>[]),
    ...
    return NextResponse.json({
      configured:firecrawlConfigured()||engineSearchConfigured(),firecrawl:firecrawlConfigured(),live_search:engineSearchConfigured(),ai:radarAIConfigured(),
      company_brain:companyBrainReadiness(workspace),ai_expansion_items:expansionRows.length,candidates,
      last_run:recentRuns[0]||null,recent_runs:recentRuns,
...
export async function POST(req:Request){
  ...
  try{
    ...
    const aiQueries=(Array.isArray(expansion?.search_queries)?expansion.search_queries:[]).map((q:any)=>cleanQuery(String(q))).filter(Boolean).slice(0,20);
    const manualQueries=baseQueries(workspace);
    const queries=[...new Set([...aiQueries,...manualQueries])].slice(0,28);
    ...
    for(const result of deduped){
      ...
      if(analysis?.companies?.length){
        for(const c of analysis.companies){
          const name=cleanCompanyName(c.name);if(!validCompanyName(name))continue;
        ...
        if(provisional>=14&&validCompanyName(name))entities.push({name,official_website:originOf(result.url),related_product:String(result.description||"").slice(0,250),relationship_reason:`Company-domain result matched a RADAR search angle: ${result.source_query}`,relation_confidence:55,product_overlap_score:provisional,source_url:result.url,source_query:result.source_query,source_type:"company"});
      }
    }
    ...
    const shortlisted=[...entityMap.values()].filter(e=>e.product_overlap_score>=14&&e.relation_confidence>=50).sort((a,b)=>b.product_overlap_score-a.product_overlap_score).slice(0,45);
    ...
    for(const entity of resolved){
      ...
      const autoEligible=Boolean(officialDomain)&&!sourceLike(officialDomain)&&entity.product_overlap_score>=50&&entity.relation_confidence>=70;
      const computedStatus=autoEligible?"promoted":"candidate";
      const existingCandidate:any=candidateMap.get(candidateDomain);
      const protectedStatus=existingCandidate?.status&&["rejected","ignored","promoted"].includes(existingCandidate.status)?existingCandidate.status:computedStatus;
      const candidatePatch={title:entity.name,url:entity.official_website||entity.source_url,domain:candidateDomain,description:entity.relationship_reason,provisional_score:provisional,status:protectedStatus,entity_type:"company",related_product:entity.related_product||null,relationship_reason:entity.relationship_reason||null,relation_confidence:entity.relation_confidence,source_page_url:entity.source_url,official_website:entity.official_website||null,product_overlap_score:entity.product_overlap_score,updated_at:new Date().toISOString()};

      if(!existingCandidate){
        const pending={workspace_id:workspace.id,source_query:entity.source_query,...candidatePatch};
        candidateRows.push(pending);candidateMap.set(candidateDomain,{__pending:true,pendingRow:pending,status:pending.status,product_overlap_score:pending.product_overlap_score});
      }else if(existingCandidate.id){
        candidateUpdates.push(sbUpdate("radar_candidates",`id=eq.${existingCandidate.id}`,candidatePatch));
      }else if(existingCandidate.pendingRow&&Number(entity.product_overlap_score||0)>Number(existingCandidate.pendingRow.product_overlap_score||0)){
        const keepStatus=["rejected","ignored","promoted"].includes(existingCandidate.pendingRow.status)?existingCandidate.pendingRow.status:computedStatus;
        Object.assign(existingCandidate.pendingRow,candidatePatch,{status:keepStatus});
      }

      const reviewBlocked=["rejected","ignored"].includes(String(existingCandidate?.status||""));
      if(autoEligible&&!reviewBlocked&&officialDomain!==ownDomain&&!existingDomains.has(officialDomain)&&!existingNames.has(entity.name.toLowerCase())){
    ...
    return NextResponse.json({ok:true,search:{firecrawl:firecrawlConfigured(),live_search:engineSearchConfigured()},company_brain_expansion:{used:Boolean(aiQueries.length),cached:Boolean(expansion?.cached),provider:expansion?.provider||null,model:expansion?.model||null,queries:aiQueries.length,error:expansion?.error||null},queries_generated:queries.length,inspected,entities_extracted:shortlisted.length,candidates_found:insertedCandidates.length,promoted:insertedCompetitors.length,competitors:insertedCompetitors,candidates:insertedCandidates.sort((a:any,b:any)=>Number(b.product_overlap_score||0)-Number(a.product_overlap_score||0)).slice(0,75)});
  }catch(error){

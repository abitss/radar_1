...
function scoreCandidate(text: string, workspace: any) {
  ...
  for (const [items, weight] of groups) {
    ...
    if(unique.length) score += (unique.filter(term=>lower.includes(term)).length/unique.length)*weight;
  }
  return Math.min(100, Math.round(score));
}
function buildQueries(workspace:any){
  const product=normalizeTerms(workspace.product_keywords).slice(0,5), caps=normalizeTerms(workspace.capability_keywords).slice(0,6), tech=normalizeTerms(workspace.technology_keywords).slice(0,3), customer=[...normalizeTerms(workspace.target_customers),...normalizeTerms(workspace.buyer)].slice(0,4), problem=normalizeTerms(workspace.problem_statement).slice(0,5);
  ...
  for(const cap of caps.slice(0,3))q.add(`${cap} ${customer.slice(0,2).join(" ")||"startup company"}`);
...
export async function POST(req:Request){
  ...
  try{
    ...
    const [existingQueries,existingCompetitors,existingCandidates] = await Promise.all([
      ...
      sbSelect(`radar_competitors?workspace_id=eq.${workspace.id}&select=website`),
      sbSelect(`radar_candidates?workspace_id=eq.${workspace.id}&select=domain`),
    ]);
    ...
    const candidateDomains=new Set(existingCandidates.map((x:any)=>x.domain));

    // Search all discovery paths concurrently. Metadata-only search is much faster than
    // scraping every result before relevance has been established.
    const searches = await Promise.allSettled(queries.map(query=>searchWebFast(query,5).then(results=>({query,results}))));
    let inspected=0;
    ...
    await sbUpdate("radar_scan_runs",`id=eq.${run.id}`,{status:"completed",pages_scanned:inspected,findings:competitorRows.length,finished_at:new Date().toISOString()});
    return NextResponse.json({ok:true,queries_generated:queries.length,inspected,candidates_found:insertedCandidates.length,promoted:competitorRows.length,candidates:insertedCandidates.sort((a:any,b:any)=>Number(b.provisional_score||0)-Number(a.provisional_score||0)).slice(0,50)});
  }catch(error){

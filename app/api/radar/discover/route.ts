...
function scoreCandidate(text: string, workspace: any) {
  ...
  const groups: Array<[string[], number]> = [
    [normalizeTerms(workspace.product_keywords), 28],
    [normalizeTerms(workspace.capability_keywords), 22],
    [normalizeTerms(workspace.technology_keywords), 10],
    [normalizeTerms(workspace.target_customers), 16],
    [normalizeTerms(workspace.buyer), 8],
    [normalizeTerms(workspace.problem_statement), 12],
    [normalizeTerms(workspace.business_model), 4],
  ];
  let score = 0;
  for (const [items, weight] of groups) {
    const unique = [...new Set(items.map(x => x.toLowerCase()))];
    if (!unique.length) continue;
    score += (unique.filter(term => lower.includes(term)).length / unique.length) * weight;
  }
  return Math.min(100, Math.round(score));
}
function buildQueries(workspace: any) {
  const product = normalizeTerms(workspace.product_keywords).slice(0, 6);
  const caps = normalizeTerms(workspace.capability_keywords).slice(0, 8);
  const tech = normalizeTerms(workspace.technology_keywords).slice(0, 5);
  const customer = [...normalizeTerms(workspace.target_customers), ...normalizeTerms(workspace.buyer)].slice(0, 4);
...
export async function GET(req: Request) {
  try {
    ...
    const candidates = await sbSelect(`radar_candidates?workspace_id=eq.${workspace.id}&select=*&order=provisional_score.desc&limit=100`);
    return NextResponse.json({ configured: firecrawlConfigured(), candidates });
  } catch (error) {
...
export async function POST(req: Request) {
  try {
    ...
    const existingQueries = await sbSelect(`radar_discovery_queries?workspace_id=eq.${workspace.id}&select=query`);
    ...
    const existingCandidates = await sbSelect(`radar_candidates?workspace_id=eq.${workspace.id}&select=domain`);
    ...
    for (const query of queries.slice(0, 12)) {
      ...
      for (const result of results) {
        ...
        seen.add(domain);
        const text = `${result.title || ""} ${result.description || ""} ${result.markdown || ""}`.slice(0, 40000);
        ...
        if (provisional >= 24 && !existingDomains.has(domain)) {
          await sbInsert("radar_competitors", {
            workspace_id: workspace.id,
            name: String(result.title || domain).split(/[|–—-]/)[0].trim().slice(0, 80),
            website: originOf(result.url),
            description: result.description || null,
            category: provisional >= 80 ? "direct" : provisional >= 55 ? "adjacent" : provisional >= 30 ? "micro" : "emerging",
            similarity_score: provisional,
            threat_score: Math.min(100, Math.round(provisional * 1.08)),
            momentum_score: 35,
            movement: "stable",
            why_it_matters: `Automatically discovered from public web evidence with provisional similarity ${provisional}%.`,
          });

...
export async function POST(req: Request) {
  try {
    ...
    const updated = await sbUpdate("radar_workspaces", `id=eq.${workspace.id}`, {
      ...
      buyer: profile.buyer || "",
      product_keywords: Array.isArray(profile.product_keywords) ? profile.product_keywords.slice(0, 16) : [],
      capability_keywords: Array.isArray(profile.capability_keywords) ? profile.capability_keywords.slice(0, 20) : [],
      technology_keywords: Array.isArray(profile.technology_keywords) ? profile.technology_keywords.slice(0, 16) : [],
      major_features: Array.isArray(profile.major_features) ? profile.major_features.slice(0, 20) : [],

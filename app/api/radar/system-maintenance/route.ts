...
export async function POST(req:Request){
  try{
    ...
    const stale:any[]=await sbSelect(`radar_jobs?job_type=eq.initial_intelligence&status=eq.running&started_at=lt.${encodeURIComponent(staleIso)}&select=id,workspace_id,attempts,max_attempts&limit=20`);
    for(const job of stale){
      const terminal=Number(job.attempts||0)>=Number(job.max_attempts||3);
      await sbUpdate("radar_jobs",`id=eq.${job.id}`,{
        status:terminal?"failed":"queued",
        current_step:terminal?"failed":"recovered_after_timeout",
        error:"Recovered after a stale worker timeout.",
        available_at:terminal?nowIso:new Date(Date.now()+60000).toISOString(),
        updated_at:nowIso,
      }).catch(()=>{});
    }
    ...
    for(const job of queued){
      const r=await systemCall(req,String(job.workspace_id),"/api/radar/initialize",{resume:true});
    ...
    const dueTasks:any[]=await sbSelect(`radar_recurring_tasks?next_run_at=lte.${encodeURIComponent(nowIso)}&select=workspace_id,task_key,next_run_at&order=next_run_at.asc&limit=30`);
    const allDueWorkspaceIds=Array.from(new Set<string>(dueTasks.map((task:any)=>String(task.workspace_id))));
    ...
    for(const workspaceId of dueWorkspaceIds){
      ...
      maintenanceResults.push({
        ...
        ok:r.ok,
        status:r.status,
        tasks_processed:Number(r.data?.tasks_processed||0),
        tasks_failed:Number(r.data?.tasks_failed||0),
        error:r.ok?null:r.data?.error||"Maintenance failed",
      });
    }
    ...
    return NextResponse.json({
      ok:true,
      at:nowIso,
      stale_jobs_recovered:stale.length,
      workspaces_registered:workspaces.length,

...
export async function POST(req:Request){
  ...
  try{
    ...
    if(!readiness.ready)return NextResponse.json({ok:false,error:`Company Brain is incomplete. Missing: ${readiness.missing.join(", ")}.`,stage:"company_brain",company_brain:readiness},{status:400});
    const body:any=await req.json().catch(()=>({}));
    const runDecision=body?.runDecision!==false;
    const stages:any[]=[];

    const aiStatus=radarEngineAIStatus();
    if(!aiStatus.configured){stages.push({stage:"ai",ok:false,error:"No AI provider configured",status:aiStatus});return NextResponse.json({ok:false,workspace_id:workspace.id,stages,duration_ms:Date.now()-started},{status:503})}
    try{
    ...
    }catch(error){stages.push({stage:"ai",ok:false,error:error instanceof Error?error.message:"AI health failed",status:aiStatus});return NextResponse.json({ok:false,workspace_id:workspace.id,stages,duration_ms:Date.now()-started},{status:503})}

    if(workspace.website){
      const bootstrap=await internal(req,"/api/radar/bootstrap",{website:workspace.website});
      stages.push({stage:"company_brain",ok:true,input:"founder_form",website_enrichment:{attempted:true,ok:bootstrap.ok,status:bootstrap.status,engine:bootstrap.data?.engine||null,error:bootstrap.ok?null:bootstrap.data?.error||null}});
    }else{
      stages.push({stage:"company_brain",ok:true,input:"founder_form",website_enrichment:{attempted:false,ok:true,optional:true}});
    }
    ...
    }catch(error){stages.push({stage:"sources",ok:false,error:error instanceof Error?error.message:"Source monitoring failed"});return NextResponse.json({ok:false,workspace_id:workspace.id,stages,duration_ms:Date.now()-started},{status:500})}
    ...
    if(runDecision&&moves[0]?.id){const decisionCall=await internal(req,"/api/radar/decisions",{moveId:moves[0].id});decision=decisionCall.data||null;stages.push({stage:"decision",ok:decisionCall.ok,status:decisionCall.status,decision_id:decision?.id||null,error:decisionCall.ok?null:decisionCall.data?.error||null});if(!decisionCall.ok)return NextResponse.json({ok:false,workspace_id:workspace.id,stages,duration_ms:Date.now()-started},{status:500})}
    else stages.push({stage:"decision",ok:true,skipped:true,reason:runDecision?"No correlated Move exists yet. Decision creation requires evidence-backed Moves.":"Disabled by test request."});
    ...
    const [competitors,sources,snapshots,signals,finalMoves,decisions]:any[]=await Promise.all([
      ...
      sbSelect(`radar_sources?workspace_id=eq.${workspaceId}&select=id&limit=2000`).then(async(rows:any[])=>{if(!rows.length)return[];const ids:string[]=rows.map((r:any)=>String(r.id||"")).filter(Boolean).slice(0,500);if(!ids.length)return[];return sbSelect(`radar_snapshots?source_id=in.(${ids.map((id:string)=>encodeURIComponent(id)).join(",")})&select=id&limit=5000`)}),
    ...
    const discoveryOk=summary.competitors>0;
    const monitoringOk=summary.sources>0&&summary.snapshots>0;
    const intelligenceOk=summary.signals>0||summary.moves>0||Number(refresh.data?.market_events||0)>0;
    const notes:string[]=[];
    if(discoveryOk&&!monitoringOk)notes.push("Competitor discovery works, but no monitorable competitor source has produced a baseline snapshot yet.");
    if(discoveryOk&&monitoringOk&&!intelligenceOk)notes.push("Discovery and source monitoring work, but no material market change was verified during this run. RADAR intentionally does not fabricate Signals.");
    return NextResponse.json({ok:discoveryOk&&monitoringOk,workspace_id:workspace.id,workspace_name:workspace.name,input:"company_brain",website_optional:true,stages,summary,intelligence_observed:intelligenceOk,duration_ms:Date.now()-started,notes});
  }catch(error){if(error instanceof Error&&error.message==="UNAUTHORIZED")return NextResponse.json({ok:false,error:"Unauthorized"},{status:401});return NextResponse.json({ok:false,error:error instanceof Error?error.message:"E2E intelligence test failed",duration_ms:Date.now()-started},{status:500})}
}

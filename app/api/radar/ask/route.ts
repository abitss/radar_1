...
import { generateRadarAnswer, radarAIConfigured } from "@/lib/radar-ai";

function deterministicAnswer(q: string, competitors: any[], signals: any[], recommendations: any[], evidence: any[]) {
...
export async function POST(req: Request) {
  try {
    ...
    const fallback = deterministicAnswer(q, competitors, signals, recommendations, evidence);

    let aiAnswer: string | null = null;
    if (radarAIConfigured()) {
      try {
        aiAnswer = await generateRadarAnswer({
        ...
        console.error("Ask RADAR AI fallback:", error instanceof Error ? error.message : error);
      }
    }

    return NextResponse.json({
      answer: aiAnswer || fallback.answer,
      mode: aiAnswer ? "grounded-ai" : "evidence-fallback",
      evidence:fallback.refs.map((e:any)=>({ title:e.title, url:e.source_url, fact:e.fact, confidence:e.confidence }))
    });
  } catch (error) {

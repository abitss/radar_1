...
export async function POST(req: Request) {
  try {
    ...
    for(const query of queries){ for(const row of await searchWeb(query,4)){ if(!row?.url||seen.has(row.url))continue; seen.add(row.url);pages.push(row);} if(pages.length>=15)break; }
    ...
    if(!existingMonitor[0]){
      const urls=[...new Set(pages.map(p=>p.url).filter((u:string)=>domainOf(u)===domain))].slice(0,10);
      if(urls.length){
    ...
    return NextResponse.json({similarity:weighted,threat,movement,category,matched,dimensions:dims,why,pages_scanned:pages.length,monitor_active:true});
  } catch (error) {

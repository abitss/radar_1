...
export default function CompaniesPage(){
  ...
  return <div className="content">
    ...
    <article className="panel" style={{padding:18}}>
      ...
      {loading?<div style={{padding:20,color:"#747b81"}}>Loading competitors...</div>:filtered.length?<div className="intel-table-wrap"><div className="intel-table-head" style={{gridTemplateColumns:"2fr .7fr .7fr .7fr"}}><span>Company</span><span>Similarity</span><span>Threat</span><span>Movement</span></div>{filtered.map(c=><Link href="/market" className="intel-table-row" style={{gridTemplateColumns:"2fr .7fr .7fr .7fr"}} key={c.id}><div className="signal-main"><div><span className="signal-company">{c.category}</span></div><strong>{c.name}</strong><small>{c.description||c.website}</small></div><strong>{Math.round(Number(c.similarity_score||0))}%</strong><strong>{Math.round(Number(c.threat_score||0))}%</strong><span className="age-cell">{c.movement||"stable"}<ArrowUpRight size={13}/></span></Link>)}</div>:<div style={{padding:28,textAlign:"center",color:"#737a80"}}>No competitors yet. Run onboarding once and RADAR will search the public web automatically.</div>}
    </article>
  </div>;
}

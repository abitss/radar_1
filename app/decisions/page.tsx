...
export default function DecisionsPage(){
  ...
  const decisionMoveIds=useMemo(()=>new Set(rows.filter(r=>r.status!=="dismissed").map(r=>String(r.move_id||""))),[rows]);
  ...
  return <div className="content">
    ...
    <div className="decision-stack">{loading?<div className="panel" style={{padding:24}}>Loading decisions...</div>:filtered.length?filtered.map(r=>{
      ...
      :<div className="panel decision-empty"><Target size={24}/><strong>{rows.length?"No decisions match this view.":"No Decisions yet."}</strong><span>{rows.length?"Change the filter or search term.":"RADAR needs a multi-signal Move before it can build a grounded decision memo. Open Moves to correlate signals first."}</span>{!rows.length?<Link className="secondary-button" href="/moves">Open RADAR Moves</Link>:null}</div>}</div>
  </div>
}

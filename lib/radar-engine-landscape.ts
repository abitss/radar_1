...
function terms(value:any):string[]{const rows=Array.isArray(value)?value.map(String):String(value||"").split(/[,;|\n]/);return rows.map((x:string)=>x.trim()).filter((x:string)=>x.length>2)}
...
function queries(workspace:any):string[]{
  const product=terms(workspace.product_keywords).slice(0,5);const features=terms(workspace.major_features).slice(0,4);const capabilities=terms(workspace.capability_keywords).slice(0,5);const customers=[...terms(workspace.target_customers),...terms(workspace.buyer)].slice(0,4);const industry=terms(`${workspace.industry||""},${workspace.sub_category||""}`).slice(0,3);const problem=terms(workspace.problem_statement).slice(0,5);const q=new Set<string>();
...
export async function runCompetitiveLandscape(workspaceId:string):Promise<LandscapeResult>{
  ...
  const ownDomain=domain(String(workspace.website||""));const queryList=queries(workspace);
  ...
  const evidenceMap=new Map<string,any>();for(const batch of batches){if(batch.status!=="fulfilled")continue;for(const row of batch.value as any[]){const url=String(row?.url||"");if(!url||(ownDomain&&domain(url)===ownDomain)||evidenceMap.has(url))continue;evidenceMap.set(url,row)}}
  ...
  const landscape:any=await analyzeCompetitiveLandscape(workspace,evidenceRows);const existing:any[]=await sbSelect(`radar_competitors?workspace_id=eq.${encodeURIComponent(workspaceId)}&select=*&limit=300`);const byDomain=new Map<string,any>();const byName=new Map<string,any>();for(const competitor of existing){const d=domain(String(competitor?.website||""));if(d)byDomain.set(d,competitor);const name=String(competitor?.name||"").trim().toLowerCase();if(name)byName.set(name,competitor)}
  ...
  for(const item of competitors){
    const website=origin(String(item?.website||""));const d=domain(website);if(!d||(ownDomain&&d===ownDomain))continue;
    ...
    const similarity=clamp(item?.similarity),threat=clamp(item?.threat),confidence=clamp(item?.confidence,65),productOverlap=clamp(item?.components?.product,Math.round(similarity*.75)),strategicRelevance=clamp(item?.threat_components?.strategic_relevance,similarity),momentum=clamp(item?.threat_components?.momentum,40),category=safeCategory(item?.classification),itemName=String(item?.name||d).trim().slice(0,160);const current:any=byDomain.get(d)||byName.get(itemName.toLowerCase())||null;const why=`${item?.related_product?`${itemName} makes ${String(item.related_product)}. `:""}${String(item?.rationale||item?.summary||"")} RADAR landscape similarity ${similarity}%, threat ${threat}%, confidence ${confidence}%.`;
    ...
    let competitor:any=current;if(current?.id){const rows:any[]=await sbUpdate("radar_competitors",`id=eq.${encodeURIComponent(String(current.id))}&workspace_id=eq.${encodeURIComponent(workspaceId)}`,patch);competitor=rows[0]||current;updated++}else{const rows:any[]=await sbInsert("radar_competitors",{workspace_id:workspaceId,...patch});competitor=rows[0]||null;inserted++;if(competitor){byDomain.set(d,competitor);byName.set(itemName.toLowerCase(),competitor)}}if(!competitor?.id)continue;

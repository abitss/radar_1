...
import { crawlStartupEngine, fetchSnapshotEngine, sourceTypeForUrl } from "@/lib/radar-engine-crawl";
...
const siteDiscoveryAt=new Map<string,number>();
...
export async function ensureWorkspaceSources(workspaceId:string){
  const [competitors,evidence,existing]=await Promise.all([
    ...
    sbSelect(`radar_sources?workspace_id=eq.${workspaceId}&select=id,url,competitor_id,source_type&limit=1000`),
  ]);
  ...
  const topWithWebsites=competitors.filter((c:any)=>c.monitoring_preference!=="ignore"&&c.website&&!existing.some((s:any)=>s.competitor_id===c.id&&["rss","atom"].includes(s.source_type))&&Date.now()-(siteDiscoveryAt.get(`${workspaceId}:${c.id}`)||0)>86400000).slice(0,24);
  const feedResults=await Promise.allSettled(topWithWebsites.map(async(c:any)=>({c,crawl:await crawlStartupEngine(c.website,4)})));
  for(const item of feedResults){
    if(item.status!=="fulfilled")continue;
    siteDiscoveryAt.set(`${workspaceId}:${item.value.c.id}`,Date.now());
    for(const page of item.value.crawl.pages){
      if(known.has(page.url))continue;
      known.add(page.url);
      rows.push({workspace_id:workspaceId,competitor_id:item.value.c.id,url:page.url,title:page.title,source_type:sourceTypeForUrl(page.url),reliability:90,priority:Math.max(55,Number(item.value.c.threat_score||0)),check_frequency_minutes:Number(item.value.c.threat_score||0)>=75?60:360,status:"active",health:"unknown",next_check_at:new Date().toISOString()});
    }
    for(const feed of item.value.crawl.feeds.slice(0,3)){
  ...
  if(rows.length)await sbUpsert("radar_sources",rows,"workspace_id,url",true);
...
async function fetchCurrentSnapshot(url:string){
  try{
    ...
    if(text.length>=80)return{text,contentHash:hash(text),title:direct.title||"",url:direct.url||url,provider:"direct"};
  }catch{}
...
export async function runWorkspaceSourceMonitor(workspaceId:string,limit=16){
  ...
  const competitors=await sbSelect(`radar_competitors?workspace_id=eq.${workspaceId}&select=id,name,website,threat_score,related_product,monitoring_preference&limit=200`);
  ...
  for(const source of sources){
    ...
    if(competitor?.monitoring_preference==="ignore")continue;
    // Compare-and-set lease: concurrent workers cannot check this source together.
    const lease=new Date(Date.now()+30*60000).toISOString();
    const due=source.next_check_at?`eq.${encodeURIComponent(source.next_check_at)}`:"is.null";
    const claimed=await sbUpdate("radar_sources",`id=eq.${source.id}&workspace_id=eq.${workspaceId}&next_check_at=${due}`,{next_check_at:lease});
    ...
    try{
      ...
      const saveSnapshot=()=>sbInsert("radar_snapshots",{source_id:source.id,content_hash:contentHash,content_text:text,metadata:{title:page.title,url:page.url,provider:page.provider}});
      const minutes=Math.max(30,Number(source.check_frequency_minutes||360));
      await sbUpdate("radar_sources",`id=eq.${source.id}&workspace_id=eq.${workspaceId}`,{health:"healthy",last_checked_at:new Date().toISOString(),last_success_at:new Date().toISOString(),last_error:null,next_check_at:new Date(Date.now()+minutes*60000).toISOString(),updated_at:new Date().toISOString()});
      if(!previous){await saveSnapshot();result.unchanged++;continue}
      if(normalizeSnapshotText(previous.content_text)===text){result.unchanged++;continue}
      result.changed++;
      await sbUpdate("radar_sources",`id=eq.${source.id}&workspace_id=eq.${workspaceId}`,{last_changed_at:new Date().toISOString(),updated_at:new Date().toISOString()});
      if(!competitor){await saveSnapshot();continue;}
      ...
      if(!change)throw new Error("Semantic analysis unavailable; source will be retried");
      if(!change.meaningful){await saveSnapshot();continue;}
      ...
      await sbInsert("radar_evidence",{workspace_id:workspaceId,competitor_id:competitor.id,source_url:source.url,source_type:"semantic_change",title:change.title||source.title,fact:change.new_state||change.summary,summary:change.explanation||change.summary,confidence,claim_type:change.fact_or_inference});
      ...
      result.failed++;
      await sbUpdate("radar_sources",`id=eq.${source.id}&workspace_id=eq.${workspaceId}`,{health:"error",last_checked_at:new Date().toISOString(),last_error:error instanceof Error?error.message.slice(0,500):"Source check failed",next_check_at:new Date(Date.now()+60*60000).toISOString(),updated_at:new Date().toISOString()}).catch(()=>{});
    }
  }
...
export async function scheduleWorkspaceRecurringTasks(workspaceId:string){
  const specs=[
    ...
    {task_key:"market.refresh",interval_minutes:720},
  ...
  if(rows.length)await sbUpsert("radar_recurring_tasks",rows,"workspace_id,task_key",true);
  return{added:rows.length};
}
